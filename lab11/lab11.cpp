...
const long NUM = 360;
__declspec(align(16)) float A[NUM];
__declspec(align(16)) float B[NUM];
...
float ScalarProduct(float* A, float* B, long N)
{
	float result = 0.0;
	for (int i = 0; i < N; i++)
	{
...
void doCalculation(HWND hWnd) {
	...
	{
		ScalarProduct_FPU(&result, A, B, NUM);
	}
	GetLocalTime(&st);
	ten = 60000 * (long)st.wMinute + 1000 * (long)st.wSecond + (long)st.wMilliseconds - tst;
	sprintf_s(TextBuf, "Result = %f, time = %d ", result, ten);
	...
	for (long i = 0; i<1000000; i++)
	{
		ScalarProduct_SSE(&result, A, B, NUM);
	}
	GetLocalTime(&st);
	ten = 60000 * (long)st.wMinute + 1000 * (long)st.wSecond + (long)st.wMilliseconds - tst;
	sprintf_s(TextBuf, "Result = %f, time = %d ", result, ten);
